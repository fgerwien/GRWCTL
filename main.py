...
def read_sensor(sensor):
    try:
        ...
        if 'error' in sensor_data:
            logger.log_error(f"Error reading {sensor.__class__.__name__}: {sensor_data['error']}")
        else:
            return sensor_data
    except Exception as e:
        logger.log_error(f"Exception while reading {sensor.__class__.__name__}: {str(e)}")
...
def main():
    ...
    bme280_sensor = BME280Sensor(address=PINConfig.BME280_I2C_ADDRESS)  # I2C address for BME280
